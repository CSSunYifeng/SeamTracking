...
if __name__=="__main__":
    from .default import getConfig # _C as cfg
    # from .wasp import build_wasp
    # from .wasp import WASPv2
    from .mobilenet import get_mobile_net
else:
    from models.backbones.omniSeam.default import getConfig #  _C as cfg
    # from models.backbones.movenet.wasp import build_wasp
    # from models.backbones.movenet.wasp import WASPv2
...
class HRNet(nn.Module):
    def __init__(self, cfg,type='common',with_gauss_filter=True):
        ...
        if 'STAGE2' in self.cfg['MODEL']['EXTRA']:
            ...
            self.transition1 = convs(256,48,kernel_size=3,stride=1,padding=1)
            self.bn3 = nn.BatchNorm2d(48, momentum=BN_MOMENTUM)
        ...
        # If using WASPv2
        # self.waspv2 = WASPv2('SEPARABLE', 48, 48, cfg.MODEL.NUM_JOINTS,self.m_type)
        self.conv3 = convs(48, 48, kernel_size=1, stride=1, padding=0, bias=False)
        if self.m_type == "human_pose":
            self.last_channel = cfg.MODEL.NUM_JOINTS
        else:
            self.last_channel = 48 # cfg.MODEL.NUM_JOINTS
...
def get_hrnet(type="common",with_gauss_filter=True):
    cfg = getConfig()
    if type == "3stage":
    ...
    elif type == "2stage_double":
        cfg.merge_from_file(os.path.join(ROOT,'models/backbones/omniSeam/omnipose_w48_128x96_2stage_double.yaml'))
    elif type == "1stage":
    ...
    if type == "common" or "3stage":
        model = HRNet(cfg,with_gauss_filter=with_gauss_filter)
    elif  type=='human_pose':
        model = HRNet(cfg,type)
    elif type == "lite":
        assert False
    return model