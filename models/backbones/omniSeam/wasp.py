...
class WASPv2(nn.Module):
    def __init__(self, conv_type, inplanes, planes, n_classes=17, type="human_pose"):
        ...
        if self.m_type == '1stage':
            self.conv3 = nn.Conv2d(256, 48, 1, bias=False)
            self.bn3 = nn.BatchNorm2d(48)


    def exact_feat(self, x, low_level_features):
        input = x
    ...
    def forward_seamtracking(self, x, low_level_features):
        x = self.exact_feat(x,low_level_features)
        return x

    def forward_humanpose(self, x, low_level_features):
        x = self.exact_feat(x,low_level_features)
        x = self.last_conv(x)
    ...
    def forward(self, x, low_level_features):
        if self.m_type == "human_pose":
            return self.forward_humanpose(x,low_level_features)
        else:
            return self.forward_seamtracking(x,low_level_features)
        # input = x
        # x1 = self.aspp1(x)
        # x2 = self.aspp2(x1)
        # x3 = self.aspp3(x2)
